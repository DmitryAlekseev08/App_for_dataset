...
class Classification(QMainWindow):
    def __init__(self):
        ...
        self.info = {}
        self.ui.list_intents.itemClicked.connect(self.current_intent)Fina
    ...
    # Добавление интента
    def add_intent(self):
        # Форма добавления интента
        self.dialog_add = QDialog()
        self.ui_dialog_add = Intent_Dialog('add')
        self.ui_dialog_add.setupUi(self.dialog_add)
        self.dialog_add.exec_()
        new_intent = self.ui_dialog_add.name_line.text()
        if new_intent:
    ...
    def delete_intent(self):
        ...
        # Форма удаления интента
        self.dialog_del = QDialog()
        self.ui_dialog_del = Intent_Dialog('delete')
        self.ui_dialog_del.setupUi(self.dialog_del)
        self.dialog_del.exec_()
        del_intent = self.ui_dialog_del.name_line.text()
        if del_intent:
            if not self.source_text['intents']:
    ...
    def change_intent(self):
        ...
        # Форма изменения интента
        self.dialog_chan = QDialog()
        self.ui_dialog_chan = Change_Intent_Dialog()
        self.ui_dialog_chan.setupUi(self.dialog_chan)
        self.dialog_chan.exec_()
        new_name = self.ui_dialog_chan.new_name_line.text()
        old_name = self.ui_dialog_chan.old_name_line.text()
    ...
    def open_file(self):
        ...
        try:
            ...
            ftext = eval(f.read())
            for intent in ftext['intents']:
                for num in range(0, len(ftext['intents'][intent]['responses'])):
                    ftext['intents'][intent]['responses'][num] =repr(ftext['intents'][intent]['responses'][num]).strip("'")
            self.ui.list_intents.clear()
    ...
    def app_function(self):
        QMessageBox.information(self, "Функции программы", "  Данная программа позволяет создавать и редактировать "
                                                           "датасет для моделей, занимающихся обработкой "
